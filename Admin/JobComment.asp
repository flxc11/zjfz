...
Select Case Action
Case "Up"
	MinOrder=Conn.Execute("Select Min(NavOrder) From JobComment Where NavParent="&NavParent&"")(0)
	...
	Else
		Conn.Execute("Update JobComment Set NavOrder="&NavOrder&" Where NavOrder="&NavOrder-1&" And Navparent="&Navparent&"")
		Conn.Execute("Update JobComment Set NavOrder="&NavOrder-1&" Where ID="&ID&"")
	...
	Response.End()
Case "Down"
	MaxOrder=Conn.Execute("Select Max(NavOrder) From JobComment Where NavParent="&NavParent&"")(0)
	NavOrder=ReplaceBadChar(Trim(Request("NavOrder")))
	...
	Response.End()
Case "UnPublic"
	Conn.Execute("Update JobComment Set NavLock=1 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "InPublic"
	Conn.Execute("Update JobComment Set NavLock=0 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Delete From JobComment Where ID="&AryID(i)&"")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
	Response.Write("<script>alert('删除信息成功！');window.location.href='?Page="&Page&"&ID="&ParentID&"';</script>")
	Response.End()
End Select
%>
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="JobComment.asp">人才管理内容</a></td>
...
<td width="90%" valign="top">以下为系统所有毛遂自荐内容的信息列表。<br>
...
<th width="10%" class="Right">联系人</th>
<th width="13%" class="Right">联系电话</th>
<th width="17%" class="Right">邮箱</th>
<th width="14%" class="Right">应聘岗位</th>
<th class="Right">应聘时间</th>
<th width="6%" class="Right">状态</th>
<th width="4%" class="Right"><input type="checkbox" name="chkSelectAll" onclick="doCheckAll(this)" /></th>
<th width="13%" class="Right">管理操作</th>
</tr>
<%
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql="Select * From JobComment Order By posttime desc"
...
<td class="Right"><a href="LookJobComment.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>"><%=Rs("RealName")%></a></td>
<td class="Right"><%=Rs("Tel")%></td>
<td class="Right"><%=Rs("Email")%>&nbsp;</td>
<td class="Right"><%=Rs("JobName")%>&nbsp;</td>
<td class="Right"><%=Rs("PostTime")%></td>
<td class="Right">
  <%
If Rs("JobLock")="0" Then
Response.Write("<font color='#FF0000'>未查阅</font>")
Else
Response.Write("已查阅")
End If
%></td>
<td class="Right"><input type="checkbox" name="ID" value="<%=Rs("ID")%>" /></td>
<td class="Right">&nbsp;<a href="LookJobComment.asp?ID=<%=Rs("ID")%>&amp;Page=<%=Page%>&amp;ParentID=<%=ID%>">查看</a>┊<a href="?Action=Delete&amp;ID=<%=Rs("ID")%>&amp;Page=<%=Page%>&amp;ParentID=<%=ID%>" onclick="if(!confirm('确定要将该信息删除？')) return false;">删除</a></td>
</tr>
<%
i=i+1
Rs.MoveNext
