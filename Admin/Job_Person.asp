...
Select Case Action
Case "Up"
	MinOrder=Conn.Execute("Select Min(NavOrder) From JobInfo Where NavParent="&NavParent&"")(0)
	...
	Else
		Conn.Execute("Update JobInfo Set NavOrder="&NavOrder&" Where NavOrder="&NavOrder-1&" And Navparent="&Navparent&"")
		Conn.Execute("Update JobInfo Set NavOrder="&NavOrder-1&" Where ID="&ID&"")
	...
	Response.End()
Case "Down"
	MaxOrder=Conn.Execute("Select Max(NavOrder) From JobInfo Where NavParent="&NavParent&"")(0)
	NavOrder=ReplaceBadChar(Trim(Request("NavOrder")))
	...
	Response.End()
Case "UnPublic"
	Conn.Execute("Update JobInfo Set NavLock=1 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "InPublic"
	Conn.Execute("Update JobInfo Set NavLock=0 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Delete From JobInfo Where ID In ("&AryID(i)&GetAllChild("ShopClass",AryID(i))&")")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
...
Sql="Select * From JobInfo order by PostTime asc"
...
<td class="Right"><a href="?ID=<%=Rs("ID")%>"><%=Rs("NavTitle")%></a>(<%=GetSubNavCount("JobInfo",Rs("ID"))%>)</td>
