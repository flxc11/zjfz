...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Nav_Picture.asp">图片内容维护</a><%=GetNavPath("SitePicture",ID)%></td>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:20%; text-align:right"><a href="Nav_PictureAdd.asp?ID=<%=ID%>">添加图片内容</a>&nbsp;</td>
</tr>
<tr>
...
<td width="90%" valign="top">1.以下为站点所有图片信息的列表;<br />
<%if Request.Cookies("CNVP_CMS2")("ClassPage")="true" then%>
2.单击&quot;[+]&quot;可添加相应的图片信息;<br />
3.可对图片进"编辑"、"删除"、"排序"等操作;<br/>
<%else%>
2.单击&quot;[+]&quot;可添加当前级别的图片;<br />
3.单击&quot;(数字)&quot;可添加该级别的下一子级别; <br />
4.可对图片进行子父级别间的&quot;转移&quot;、&quot;编辑&quot;、&quot;删除&quot;、&quot;排序&quot;等操作;<br>
<%end if%>
...
<th width="29%" class="Right">图片名称
<a href="Nav_PictureAdd.asp?ID=<%=ID%>">[+]</a></th>
<th width="15%" class="Right">所属类别</th>
<th width="26%" class="Right">图片说明</th>
...
Sql="Select * From SitePicture Order By NavOrder Asc"
...
<td class="Right"><a href="Nav_PictureEdit.asp?ID=<%=Rs("ID")%>&ClassID=<%=Rs("ClassID")%>"><%=Rs("NavTitle")%></a>
<%if Request.Cookies("CNVP_CMS2")("ClassPage")="false" then%>
<a href="?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>">(<%=GetSubNavCount("SitePicture",Rs("ID"))%>)</a>
<%end if%></td>
<td class="Right">
<%
	...
	if not Rs2.eof then
		...
		Response.Write(Rs2("User_NavTitle"))
		end if
		if Request.Cookies("CNVP_CMS2")("SiteVersion")="English" then
		Response.Write(Rs2("User_EnNavTtile"))
		end if
...
%>
<tr>
<th colspan="4" style="font-weight:normal">操作：
<%if Request.Cookies("CNVP_CMS2")("ClassPage")="false" then%>
<a href="javascript:ChangeParent();" style="font-weight:normal">转移</a>&nbsp;┊&nbsp;
<%end if%>
</th>
